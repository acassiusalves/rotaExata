...
export type SyncPatchInput = {
  /** E-mail sincronizado, ja normalizado. */
  email: string;
  /** Papel em `users/{uid}` hoje. `undefined` quando nao ha cadastro. */
  currentRole: string | undefined;
  /** Nome em `users/{uid}` hoje. */
  currentDisplayName: string | undefined;
  /** Nome que a conta tem no Firebase Auth. */
  authDisplayName: string | null | undefined;
};

export function buildSyncPatch(input: SyncPatchInput): SyncPatch {
